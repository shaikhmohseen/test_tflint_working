...
resource "aws_security_group" "facility_service_kinesis" {
  ...
  endpoints = [
    {
      ...
      security_group_ids = [aws_security_group.facility_service_kinesis.id]
    },
  ]
  tags = merge(var.tags, {
    service      = var.service_name
...
resource "aws_vpc_endpoint" "endpoints" {
  ...
  tags               = merge(local.tags, { Name = "${var.service_name}.vpc.endpoint" })
}
