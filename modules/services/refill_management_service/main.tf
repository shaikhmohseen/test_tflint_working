...
locals {
  service_name = "refill-management-service"
  endpoints = [
    {
...
# Imported
resource "aws_ses_domain_identity" "refillmanagement_ses_identity" {
  domain = var.rms_ses_domain
}

resource "aws_ses_domain_dkim" "refillmanagement_ses_dkim" {
  domain = var.rms_ses_domain
}
