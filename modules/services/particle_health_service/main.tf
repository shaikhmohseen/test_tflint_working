module "vpc" {
  ...
  name                 = "particle-health-service-vpc"
...
resource "aws_security_group" "particle_health_service_kinesis" {
  ...
  endpoints = [
    {
      ...
      security_group_ids = [aws_security_group.particle_health_service_kinesis.id]
    },
  ]
  tags = merge(var.tags, {
    service = local.service_name
