...
locals {
  s3_object_facility_service = "donotdelete_dummy_source.zip"
  s3_bucket                  = var.build_artifacts_bucket_arn
...
resource "aws_codepipeline" "codepipeline" {
  ...
  stage {
    ...
    action {
      name             = "Facility-Service-Service-Artifact"
      category         = "Source"
      owner            = "AWS"
      provider         = "S3"
      version          = "1"
      ...
      configuration = {
        ...
        "S3ObjectKey"          = local.s3_object_facility_service
      }
      run_order = 1
    }
