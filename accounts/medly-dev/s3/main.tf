...
data "aws_iam_policy_document" "cross_account_bucket_policy" {
  statement {
    ...
    principals {
      identifiers = [for principal in merge(var.aws_account_ids, var.lambda_arns) : principal]
      type        = "AWS"
    }
...
resource "aws_s3_bucket" "ph_uploads" {
  bucket = "${var.deployment_account}-ph-uploads"
  acl    = "private"
