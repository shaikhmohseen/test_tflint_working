...
data "terraform_remote_state" "particle_health_app" {
  backend = "s3"
  config = {
    bucket  = "medly-prod-terraform-state"
    key     = "global/particle-health-app/terraform.tfstate"
    region  = "us-east-1"
    profile = "medly-prod"
  }
}
...
resource "aws_codepipeline" "amplify_codepipeline" {
  ...
  stage {
    ...
    action {
      name             = "Particle-Health-App-Artifact"
      category         = "Source"
      owner            = "AWS"
      provider         = "S3"
      version          = "1"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        ProjectName = data.terraform_remote_state.particle_health_app.outputs.particle_health_app_amplify_release_codebuild_project
      }
    }
  }
}
